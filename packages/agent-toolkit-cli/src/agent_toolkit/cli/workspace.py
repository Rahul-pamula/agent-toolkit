...
def _validate_profiles(ws: Path) -> list[str]:
    ...
    for path in sorted(profiles_dir.glob("*.yaml")):
        ...
        if skills is not None:
            ...
            print(f"  {_yellow('⚠')}  {path.name}: profile key 'skills' is schema-supported but currently not applied by the toolkit loader.", file=sys.stderr)
        loops = data.get("loops")
        if loops is not None:
            ...
            print(f"  {_yellow('⚠')}  {path.name}: profile key 'loops' is schema-supported but currently not applied by the toolkit loader.", file=sys.stderr)
    ...
    while i < len(args):
        match args[i]:
            ...
            case "--help" | "-h":
                ...
                print("Note: profile validation may warn about schema-supported keys that the Toolkit does not currently apply.")
                return 0
            case arg if not arg.startswith("-"):
                surface = arg
                i += 1
